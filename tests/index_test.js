...
test("validate load() - should merge configs", t => {
  ...
  t.equals(loadedConfig.accessKeyId, config.accessKeyId);
  t.equals(loadedConfig.secretAccessKey, config.secretAccessKey);
...
test("validate load() - should throw on no region", t => {
  ...
  t.throws(
    () => adapter.load(),
    "Config region and apiVersion must be provided"
  );
  t.end();
});
